...
public class ActionClient extends Action{
	...
	{
		AccountDBManager accoutnDB = new AccountDBManager(conn);
		
		try {
			return accoutnDB.selectAccount(client_id);
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("error ActionClient.getAccount");
		}
		return null;
	...
	public void newDeposit(Deposit deposit,Account account,Client client) throws SQLException
	{
		...
		if(deposit.getClose_date().getTime()>cal.getTimeInMillis())
		{
			this.deposit.setType("Long");
		}else{
			this.deposit.setType("Short");
		}
